...
module.exports = {
  async up(queryInterface, _Sequelize) {
    const userIdQuery = await User.findAll({ attributes: ['id'], raw: true })
    const userId = userIdQuery.map((item) => item.id)
    const discussionIdQuery = await Discussion.findAll({
      attributes: ['id'],
      raw: true,
    })
    ...
    await queryInterface.bulkInsert(
      ...
      Array.from({ length: 80 }, () => ({
        ...
        discussion_id:
          discussionId[Math.floor(Math.random() * discussionId.length)],
        created_at: new Date(),
        updated_at: new Date(),
      }))
    )
